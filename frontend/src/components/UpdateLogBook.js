...
const UpdateLogBook = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        setInventaris(invInputs.map(ii => ({
          ...
          jumlah: ii.jumlah,
          inventaris_id: ii.inventaris_id,
  ...
  const handleSubmit = async e => {
    ...
    setLoading(true);
    try {
      await axiosInstance.put(`/api/laporan/${id}`, laporan);
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded shadow p-4 space-y-4">
        ...
        <div>
          ...
          {inventaris.map((inv, idx) => (
            <div key={inv.inv_input_id} className="flex flex-col md:flex-row md:items-center gap-2 mb-2">
              <input name="nama_barang" value={inv.nama_barang} onChange={e => handleInventarisChange(idx, e)} required placeholder="Nama Barang" className="border px-3 py-2 rounded flex-1" disabled />
        ...
        <div className="flex flex-col md:flex-row md:justify-between gap-2">
          ...
          <button type="button" onClick={handleDelete} className="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700" disabled={loading}>
            Hapus Laporan
          </button>
        </div>
      </form>
