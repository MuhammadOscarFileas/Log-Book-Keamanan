...
export default function DashboardSupervisor() {
  ...
  useEffect(() => {
    ...
    if (user && user.role === 'supervisor' && user.first_login) {
      setShowModal(true);
    ...
    <div style={{display:'flex', minHeight:'100vh'}}>
      ...
      {showModal && (
        <div style={{position:'fixed',left:0,top:0,right:0,bottom:0,background:'rgba(0,0,0,0.18)',zIndex:1002,display:'flex',alignItems:'center',justifyContent:'center'}}>
          <div style={{background:'#fff',borderRadius:16,padding:'32px 28px',boxShadow:'0 2px 16px rgba(0,0,0,0.12)',minWidth:320,maxWidth:400,display:'flex',flexDirection:'column',alignItems:'center'}}>
            <h3 style={{marginBottom:18,color:'#27bcd4',textAlign:'center'}}>Update Password Baru</h3>
            <form onSubmit={handleSubmit} style={{width:'100%',maxWidth:320,margin:'0 auto',display:'flex',flexDirection:'column',alignItems:'center'}}>
              <div style={{position:'relative',marginBottom:16,width:'100%'}}>
                <input
                  ...
                  style={{width:'100%',padding:'10px 40px 10px 10px',borderRadius:8,border:'1.5px solid #d0e3ef',fontSize:16,boxSizing:'border-box'}}
                  disabled={loading}
                />
                <span
              ...
              <button
                type="submit"
                style={{width:'100%',maxWidth:320,background:'#27bcd4',color:'#fff',padding:'10px',border:'none',borderRadius:8,fontWeight:600,fontSize:16,cursor:loading?'not-allowed':'pointer',margin:'0 auto'}}
                disabled={loading}
              >
                Simpan Password
              </button>
